<x-app-layout :title="$title" :description="$description">
    ...
    <div class="py-2 flex items-center justify-between ml-4">
        {{-- Kembali --}}
        <div>
            <a href="{{ route('admin.schedules.index') }}" class="inline-flex items-center px-4 py-2 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md font-semibold text-xs text-gray-700 dark:text-gray-300 uppercase tracking-widest shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 transition ease-in-out duration-150">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6">
                    ...
                    <form method="GET" action="{{ route('admin.schedules.timetable.index') }}"
                                                    ...
                                                    @if ($cellEntry)
                                                        <div
                                                            ...
                                                            <div
                                                                ...
                                                                <button type="button"
                                                                    onclick="editEntry('{{ $cellEntry->id }}', '{{ $cellEntry->teacher_subject_id }}', '{{ $cellEntry->room_history_id }}', {{ $dayNum }}, '{{ $period->id }}')"
    ...
    <x-modal name="add-entry-modal" focusable>
        <div class="p-6">
            ...
            <form id="addEntryForm" method="POST" action="{{ route('admin.schedules.entries.store') }}"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Mata
                        Pelajaran *</label>
                    <select id="addSubject" required
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Guru
                        (Tersedia) *</label>
                    <select name="teacher_subject_id" id="addTeacherSubject" required disabled
                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm disabled:opacity-60">
                        <option value="">Pilih mapel terlebih dahulu</option>
                    </select>
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
                        Guru yang tampil hanya yang tidak bentrok pada hari dan jam ini.
                    </p>
                </div>
    ...
    <script>
        ...
        const busyTeachersBySlot = @json($busyTeachersBySlot ?? []);
        let currentAddSlotKey = null;

        const addSubjectSelect = document.getElementById('addSubject');
        const addTeacherSelect = document.getElementById('addTeacherSubject');
        const addRoomHistorySelect = document.getElementById('addRoomHistory');

        const editSubjectSelect = document.getElementById('editSubject');
        const editTeacherSelect = document.getElementById('editTeacherSubject');
        const editRoomHistorySelect = document.getElementById('editRoomHistory');

        const subjectOptions = Array.from(
            new Map(teacherSubjectOptions.map((option) => [option.subject_id, option.subject_name]))
                .entries()
        ).map(([id, name]) => ({
            id,
            name
        ...
        function resetAddTeacherSelect(placeholder = 'Pilih mapel terlebih dahulu') {
            if (!addTeacherSelect) {
                return;
            }

            addTeacherSelect.innerHTML = '';
        ...
        function resetEditTeacherSelect(placeholder = 'Pilih mapel terlebih dahulu') {
        ...
        function initializeAddSubjectOptions() {
            if (!addSubjectSelect) {
                return;
            }
            ...
            addSubjectSelect.appendChild(defaultOption);
        ...
        function initializeEditSubjectOptions() {
            ...
            defaultOption.value = '';
            defaultOption.textContent = 'Pilih Mata Pelajaran';
            editSubjectSelect.appendChild(defaultOption);
        ...
        function updateEditAvailableTeachersBySubject() {
            if (!editSubjectSelect || !editTeacherSelect) {
                return;
            }

            const selectedSubjectId = editSubjectSelect.value;

            if (!selectedSubjectId) {
                editTeacherSelect.disabled = true;
                resetEditTeacherSelect();
                return;
            }

            // For edit modal, show all teachers for the subject (don't filter by busy)
            const availableTeachers = teacherSubjectOptions
                .filter((option) => option.subject_id === selectedSubjectId)
                .sort((first, second) => first.teacher_name.localeCompare(second.teacher_name, 'id'));

            editTeacherSelect.disabled = false;
            editTeacherSelect.innerHTML = '';

            if (!availableTeachers.length) {
                const noTeacherOption = document.createElement('option');
                noTeacherOption.value = '';
                noTeacherOption.textContent = 'Tidak ada guru untuk mata pelajaran ini';
                editTeacherSelect.appendChild(noTeacherOption);
                return;
            }

            const defaultTeacherOption = document.createElement('option');
            defaultTeacherOption.value = '';
            defaultTeacherOption.textContent = 'Pilih Guru';
            editTeacherSelect.appendChild(defaultTeacherOption);

            availableTeachers.forEach((option) => {
                const teacherOption = document.createElement('option');
                teacherOption.value = option.id;
                teacherOption.textContent = option.teacher_name;
                editTeacherSelect.appendChild(teacherOption);
            });
        }

        function updateAvailableTeachersBySubject() {
            if (!addSubjectSelect || !addTeacherSelect) {
                return;
            }

            const selectedSubjectId = addSubjectSelect.value;

            if (!selectedSubjectId) {
                addTeacherSelect.disabled = true;
                resetAddTeacherSelect();
                return;
            }

            const busyTeacherIds = new Set(busyTeachersBySlot[currentAddSlotKey] || []);
            const availableTeachers = teacherSubjectOptions
                .filter((option) => option.subject_id === selectedSubjectId && !busyTeacherIds.has(option.teacher_id))
                .sort((first, second) => first.teacher_name.localeCompare(second.teacher_name, 'id'));
            ...
            if (!availableTeachers.length) {
                const noTeacherOption = document.createElement('option');
                noTeacherOption.value = '';
                noTeacherOption.textContent = 'Tidak ada guru tersedia di jam ini';
                addTeacherSelect.appendChild(noTeacherOption);
                return;
            }
            ...
            defaultTeacherOption.value = '';
            defaultTeacherOption.textContent = 'Pilih Guru';
            addTeacherSelect.appendChild(defaultTeacherOption);

            availableTeachers.forEach((option) => {
                const teacherOption = document.createElement('option');
                teacherOption.value = option.id;
                teacherOption.textContent = option.teacher_name;
                addTeacherSelect.appendChild(teacherOption);
            });
        }

        if (addSubjectSelect) {
            addSubjectSelect.addEventListener('change', updateAvailableTeachersBySubject);
        }

        if (editSubjectSelect) {
            editSubjectSelect.addEventListener('change', updateEditAvailableTeachersBySubject);
        }

        initializeAddSubjectOptions();
        initializeEditSubjectOptions();
        ...
        function addEntry(templateId, dayOfWeek, periodId) {
            ...
            currentAddSlotKey = `${dayOfWeek}-${periodId}`;
            if (addSubjectSelect) {
                addSubjectSelect.value = '';
            }
            if (addRoomHistorySelect) {
                addRoomHistorySelect.value = '';
            }
            if (addTeacherSelect) {
                addTeacherSelect.disabled = true;
                resetAddTeacherSelect();
        ...
        function editEntry(entryId, teacherSubjectId, roomHistoryId, dayOfWeek, periodId) {
            const form = document.getElementById('editEntryForm');
            form.action = `/admin/schedules/entries/${entryId}`;

            // Preselect subject and populate teacher list, then set current teacher
            if (editSubjectSelect && editTeacherSelect) {
                const tsOption = teacherSubjectOptions.find(o => String(o.id) === String(teacherSubjectId));
                if (tsOption) {
                    editSubjectSelect.value = tsOption.subject_id;
                    updateEditAvailableTeachersBySubject();
                    editTeacherSelect.value = teacherSubjectId || '';
                } else {
            ...
            // Display day and period info
            const dayName = days[dayOfWeek] || 'Hari';
            const editEntryInfoDiv = document.querySelector('[id="addEntryInfo"]');
            if (editEntryInfoDiv) {
                editEntryInfoDiv.innerHTML = `<strong>Hari:</strong> ${dayName}`;
            }
